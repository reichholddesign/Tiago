<?php

include 'header.php';
?>
    
        <main class="main-ctn">
       
        <div class="arrow-down"></div>


            <section  class="hero-section">
                <div class="home-img-overlay">
             
                </div>
                <div class="freelance-ctn">
             
                    <p>Freelance Illustrator</p>
                </div>

            </section>


            <section  class="about-ctn">

             <div class="about--txt-ctn">   
                <h3>About</h3>
                <p id="about">Tiago is a bilingual freelance Illustrator and comic artist currently based in Kumamoto, Japan. His passion for Illustration, comics and design stems from an interest in people and the desire to capture the beauty of life on a blank page.</p>
              
                <div class="about-main-img">
                </div> 
               

            </div>
            
            </section>
            
            <section class="client-section">
            
                <div class="clients-ctn">
                    <div class="clients"><h3 class="client-hdr">Clients</h3></div>
                    <div class="clients client-1"></div> <div class="clients client-2"></div> <div class="clients client-3"></div> <div class="clients client-4"></div>
                </div>
            
            
                <div class="girl-on-train-img"></div>
                <div class="tiago-quote-ctn">
                    <h2 class="tiago-quote">“Needing a unique present for my fiancée's 30th birthday, I reached out to Tiago to commission a drawing. I was completely thrilled with it. It was an excellent gift!”</h2>
                 <p class="tiago-quoter">Laura Reed, recent client</p>   
                </div>


            </section>
      
           
            <section class="self-qoute-section">

                <div class="self-quote-ctn">    
                 <div class="self-quote-content-wrp">   
                <img class="self-img" src="img/tiago-personal-img/tiago-work-img.png" alt="Cartoon image of Tiago Mattis">
                <h2 class="self-quote-txt">"I’m a hard worker and I enjoy taking on different challenges."</h2>
                </div>

                </div>
               

                <div class="self-quote-ctn-left fade-in-over-txt">    
                    <p class="skills-hdr">Skills</p>
                    <div class="skill-ctn">
                    <i class="fas fa-pencil-alt"></i>
                    <p>Sketching / Storyboarding</p>
                    <div class="skill-rating-ctn"><div class="sketching"></div></div>
                    </div>
                    <div class="skill-ctn">
                    <i class="fas fa-mouse"></i>
                    <p>Photoshop, Illustrator & inDesign</p>
                    <div class="skill-rating-ctn"><div class="adobe"></div></div>
                    </div>
                    <div class="skill-ctn">
                    <i class="fas fa-crop-alt"></i>
                    <p>Graphic Design</p>
                    <div class="skill-rating-ctn"><div class="graphic"></div></div>
                    </div>
                    
                    <div class="skill-ctn">
                    <i class="far fa-comments"></i>
                    <p>Fluent in English and Japanese</p>
                    <div class="skill-rating-ctn"><div class="fluency"></div></div>
                    </div>
                    <p class="skills-hdr interest-ctn">Interests</p>
                    <div class="skill-ctn">
                    <i class="fas fa-film"></i>
                    <p>Photography & Film</p>
                    </div>
                    <div class="skill-ctn">
                    <i class="fas fa-music"></i>
                    <p>Music</p>
                    </div>
                    
                    <div class="skill-ctn">
                    <i class="fas fa-plane"></i>
                    <p>Travel</p>
                    </div>        
                </div>

                <div class="self-quote-ctn-right fade-in-over-txt">  
                <p class="skills-hdr award-text">Qualifications & Awards</p>   
                <ul>
                    <li><p><span class="red-text">Rose of York言語カレッジ</span><br>TESOL (Merit)<br>ロンドン、イングランド<br>2015</p></li>
                    <li><p><span class="red-text">JLPT 日本語能力試験N2</span><br>読書: A 文法:A<br>2018</p></li>
                    <li><p><span class="red-text">Vodaphone デザイン大会</span><br>次点者賞<br>ロンドン、イングランド<br>2012</p></li>
                    <li><p><span class="red-text">Fumetto 国際コミックフェス</span><br>次点者賞<br>スイス<br>2013</p></li>
                    <li><p><span class="red-text">Cheltenham Illustration awards</span><br>次点者賞<br>チェルトネム、イングランド<br>2011</p></li>

                </ul>    
                </div>


            
            </section>



        
            
                
            <section class="about-contact-section">

                <div class="about-bg">
                <div class="contact-tiago-img"></div>
                <h2 class="work-tog-txt">“If you have a project you’d like to bring to life, let’s work together!”</h2>
                
                <div class="form-wrp">     
        <h3>Contact</h3>   
            <form class="form-ctn" action="">
            <input class="std-form-style" type="text" placeholder=" Name">
            <input class="std-form-style" type="email" placeholder=" E-mail">
            <textarea class="form-style" placeholder="Message" rows = "3" cols = "60" name = "description"></textarea>          
             <button class="no-button-style" type="submit"><i class="fas fa-paper-plane"></i></button>
            </form>
        </div>
        </div>

            </section>

        </main>

  

<script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/2.1.3/TweenMax.min.js" integrity="sha256-lPE3wjN2a7ABWHbGz7+MKBJaykyzqCbU96BJWjio86U=" crossorigin="anonymous"></script>    
<script src="https://cdnjs.cloudflare.com/ajax/libs/ScrollMagic/2.0.7/ScrollMagic.min.js" integrity="sha256-2p2tRZlPowp3P/04Pw2rqVCSbhyV/IB7ZEVUglrDS/c=" crossorigin="anonymous"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/ScrollMagic/2.0.7/plugins/animation.gsap.js" integrity="sha256-peenofh8a9TIqKdPKIeQE7mJvuwh+J0To7nslvpj1jI=" crossorigin="anonymous"></script>
<script src="https://cdnjs.cloudflare.com/ajax/libs/ScrollMagic/2.0.7/plugins/debug.addIndicators.js" integrity="sha256-31FC/OT6XpfjAhj9FuXjw5/wPXXawCAjJQ29E23/XPk=" crossorigin="anonymous"></script>
<script src="./nav.js"></script>
<script src="./app.js"></script>    
</body>
</html>




