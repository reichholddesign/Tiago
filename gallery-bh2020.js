...
const blackHeroTitles20 = ['BarackObama', 'CathayWilliams', 'ChuckBerry', 'ColinKaepernick', 'DizzyGillespie', 'DrMaeJemison', 'EmmettTill', 'FannieLouHamer', 'GarrettMoran', 'GuionBluford', 'IdaWells', 'JackieRobinson', 'JohnWitherspoon', 'JosephineBaker', 'KaliefBrowder', 'LeonardBrownDenverSmith', 'MalcolmX', 'MaryJackson', 'MayaAngelou', 'NelsonMandela', 'NinaSimone', 'PatrickClark', 'RedFoxx', 'RichardPryor', 'RobertAbbott', 'RosaParks', 'SamCooke', 'ThurgoodMarshall', 'TrayvonMartin', 'UsainBolt'];
...
function loadBlackHero2019Gallery() {
    ...
    blackHeroTitles20.forEach(function(title) {
...
Modernizr.on('webp', function (result) {
    if (result) {
        garlleryImg.style.backgroundImage = 'url(img/illustrations/DailyBlackHero_20/webp/mobile/' + title + '.webp)';
        createDiv.addEventListener("click", function(){
            viewImg(newClasstitle, title);
        } );
    }
    else {
        garlleryImg.style.backgroundImage = 'url(img/illustrations/DailyBlackHero_20/jpg/mobile/' + title + '.jpg)';
        createDiv.addEventListener("click", function(){
            viewImg(newClasstitle, title);
        } );
    }
...
if(window.innerWidth < 800) {
    ...
    function comicTouchStop(evt) {
        ...
        if(change > 0 && convertGalStrToNum < 29) {
        ...
        Modernizr.on('webp', function (result) {
            if (result) {
                getCurrentGalleryImg.style.backgroundImage = 'url(img/illustrations/DailyBlackHero_20/webp/desktop/' + blackHeroTitles20[convertGalStrToNum] + '.webp)';
               
            }
            else {
                getCurrentGalleryImg.style.backgroundImage = 'url(img/illustrations/DailyBlackHero_20/jpg/desktop/' + blackHeroTitles20[convertGalStrToNum] + '.jpg)';
              
            }
          });
    
    
          if(convertGalStrToNum === 29) {
...
nxtPageChevron.addEventListener("click", () =>{
    ...
    Modernizr.on('webp', function (result) {
        if (result) {
            getCurrentGalleryImg.style.backgroundImage = 'url(img/illustrations/DailyBlackHero_20/webp/mobile/' + blackHeroTitles20[convertGalStrToNum] + '.webp)';
           
        }
        else {
            getCurrentGalleryImg.style.backgroundImage = 'url(img/illustrations/DailyBlackHero_20/jpg/mobile/' + blackHeroTitles20[getCurrentGalleryImg] + '.jpg)';
