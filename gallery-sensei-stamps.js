...
if(window.innerWidth < 800) {
    ...
    function comicTouchStop(evt) {
        ...
        if(change > 0 && convertGalStrToNum < 16) {
        ...
        Modernizr.on('webp', function (result) {
            if (result) {
                getCurrentGalleryImg.style.backgroundImage = 'url(img/illustrations/T-senseiStamps/webp/' + senseiTitles[convertGalStrToNum] + '.webp)';

               
            }
            else {
                getCurrentGalleryImg.style.backgroundImage = 'url(img/illustrations/T-senseiStamps/jpg/' + senseiTitles[getCurrentGalleryImg] + '.jpg)';
